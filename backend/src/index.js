...
const normalizeWeaviateClassNames = (classNames, fallbackClassName) => {
...
const splitTextByCharacterWindow = (content, chunkChars, overlapChars) => {
    ...
    while (start < content.length) {
        ...
        if (hardEnd < content.length) {
            ...
            const sentenceBreak = Math.max(breakWindow.lastIndexOf('. '), breakWindow.lastIndexOf('? '), breakWindow.lastIndexOf('! '));
...
const extractSearchPhrases = (query) => {
    const quotedTerms = Array.from(query.matchAll(/"([^"]{3,})"/g))
...
const extractRelevantSnippet = (content, query, maxLength) => {
...
const normalizeWeaviateMatches = (matches, className, query, scoreLabel, snippetLength) => matches
...
const mergeWeaviateResults = (...resultSets) => {
    const byId = new Map();
...
const rerankWeaviateResults = (query, items, limit) => {
...
const searchWeaviateContexts = async (query, classNames, limit) => {
...
const buildPromptWithWeaviateContext = async (message, classNames, useWeaviateContext = true, fallbackClassName, contextOptions = normalizeContextOptions()) => {
...
const stopActiveStream = (ws, reason = 'Stopped by user') => {
    const activeStream = ws.activeOllamaStream;
    if (!activeStream) {
        return;
    }
        ...
        try {
            ...
            if (type === 'message') {
                ...
                try {
                    ...
                    const { prompt, classNames: contextClassNames, contextCount, contextItems, } = await buildPromptWithWeaviateContext(text, classNames, useWeaviateContext, className, contextOptions);
                    if (activeStream.stopped || activeStream.abortController.signal.aborted || ws.readyState !== WS_OPEN) {
                        return;
                    }
                    ...
                    response.data.on('data', (chunk) => {
                        if (isStreamComplete || activeStream.stopped || ws.readyState !== WS_OPEN)
                            return;
                        chunkCount++;
...
app.delete('/api/weaviate/collections/:className/files/:id', async (req, res) => {
    try {
        ...
        const matchingChunks = await weaviateClient.graphql
            ...
            .withLimit(10000)
            .do();
        const chunkIds = (matchingChunks?.data?.Get?.[className] || [])
            .map((chunk) => chunk._additional?.id)
            .filter((chunkId) => typeof chunkId === 'string');
        await Promise.all(chunkIds.map((chunkId) => weaviateClient.data
            .deleter()
...
app.post('/api/chat', trackAskedQuestionMiddleware, async (req, res) => {
    try {
        ...
        const resolvedModel = await resolveOllamaModel(model);
        const { prompt, classNames: contextClassNames, contextCount, contextItems, } = await buildPromptWithWeaviateContext(message, classNames, useWeaviateContext, className, contextOptions);
        const response = await axios.post(`${OLLAMA_BASE_URL}/api/generate`, {
            model: resolvedModel.model,
            prompt,
            stream: false,
...
app.post('/api/pull-model', async (req, res) => {
    try {
        ...
        const response = await axios.post(`${OLLAMA_BASE_URL}/api/pull`, { name: model }, { responseType: 'stream' });
